...
async fn handler(req: LambdaEvent<Value>) -> Result<(), Box<Error>> {
    let events: S3Event = serde_json::from_value(req.payload).unwrap();
    for e in events.records {
        ...
        let (remote_uri, buffer) = tokio::task::spawn_blocking(move || {
